...
def _build_gstr1_operations(*, filing, prepared_return, transactions, gstin_value, period_code):
    ...
    return {
        ...
        "proceed": {
            ...
            "type": "GSTR1",
            "isNil": "Y" if not save_payload.get("b2b") and not save_payload.get("b2cs") else "N",
        },
        "file": file_payload,
...
def _build_gstr1_retsave_payload(*, prepared_return, transactions, gstin_value, period_code):
    ...
    credit_notes = [item for item in transactions if item.transaction_type == "credit_note"]
    debit_notes = [item for item in transactions if item.transaction_type == "debit_note"]
    b2b_transactions = [item for item in sales_transactions if item.counterparty_gstin]
    b2c_transactions = [item for item in sales_transactions if not item.counterparty_gstin]
    ...
    return {
        ...
        "b2cs": _build_gstr1_b2cs_rows(b2c_transactions),
...
def _build_gstr1_b2cs_rows(transactions):
    ...
    for transaction in transactions:
        ...
        entry = grouped.setdefault(
            key,
            {"sply_ty": "INTER" if transaction.igst_amount else "INTRA", "pos": place_of_supply, "rt": rate, "txval": Decimal("0.00"), "iamt": Decimal("0.00"), "camt": Decimal("0.00"), "samt": Decimal("0.00"), "csamt": Decimal("0.00")},
        )
