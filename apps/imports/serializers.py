...
def infer_import_type_from_filename(file_name):
    ...
    if "gstr" in normalized and "7" in normalized:
        return ImportBatch.ImportType.TDS_DEDUCTED
    if "tds" in normalized and any(token in normalized for token in {"deduct", "deducted", "gst_tds"}):
        return ImportBatch.ImportType.TDS_DEDUCTED
